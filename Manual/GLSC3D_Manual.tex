...
\Large{制作(Ver. 2.2.1以降)：舘入 数磨$^{*_1}$，須志田 隆道$^{*_2}$，小林 亮$^{*_3}$} \\
$^*$北海道大学大学院 理学院 数学専攻,  $^{*_2}$北海道大学 電子科学研究所,  \\$^{*_3}$広島大学 数理分子生命理学専攻
\vspace{1zw}\\ 
...
%標準座標系：標準座標系とは，ディスプレイ左上を(0.0,0.0)とするピクセル単位\footnote{MacBookProなどのようなRetina Displayを搭載する機種は，必ずしも表示される映像は指定したピクセルとはなりません．従って，標準座標の数値は，厳密にはピクセルを指しません．ただし，多くの環境では標準座標の数値=ピクセルと考えてもらって良いです．}の座標系のこと．GLSC2Dでも同様の用語として使用されています．2Dもしくは3Dオブジェクトを描画する際には必要不可欠な座標系です．標準座標系では，ディスプレイ左からディスプレイ右への方向，ディスプレイ上からディスプレイ下への方向をそれぞれ正の方向とします．それぞれの方向への幅を指定する必要があります．
...
%GLSC2Dには描画オブジェクト空間という概念はありませんので，GLSC2Dユーザは注意をしてください．
...
なお，この図では $y^{f}$ 軸が上を向いているように見えますが，描画オブジェクト空間自体
...
%この座標系における3つの基本ベクトル$(1, 0, 0), (0, 1, 0), (0, 0, 1)$ の長さは全て自由に取れることに留意してください．
...
\end{eqnarray*}
この自由座標系の定義は \verb|g_def_scale_3D| という重要な関数を用いて行います（p.** ）．

\begin{figure}[htb]
\centering
\includegraphics[width=100mm]{Def_Scale.eps}
...
２次元グラフィクスにおける自由座標系の定義に関しては  \verb|g_def_scale_2D| を参照してください（p.***）\\

%・固定座標系：3Dオブジェクトが描画される座標系です．GLSC2Dではこの概念はありませんので，GLSC2Dユーザは注意をしてください．\footnote{正確には，2Dでは固定座標系と標準座標系を同一視することができるため，わざわざ固定座標系という概念を持ち出さなくても，標準座標系だけで話が完結していましたが，GLSC3D 3.0.0以降では，本概念が必須となりました．}

%3Dオブジェクトの場合，そのオブジェクトは最終的に\begin{center}\verb|{{x_f_0, x_f_1}, {y_f_0, y_f_1}, {z_f_0, z_f_1}}|\end{center}として指定される直方体の内部に描画されます．添字``\verb|_f|"は固定(fix)を示します．この座標系は，ユーザーが最終的に3Dオブジェクトを``どのように見たい"かに依存して変わります．例えば，\begin{center}\verb|{{0, 1}, {0, 1}, {0, 1}}|\end{center}と指定すると，3Dオブジェクトを右手系の立方体の中に押し込めて視ることになります．\begin{center}\verb|{{0, 1}, {0, 1}, {0, 0.5}}|\end{center}と指定すると，3Dオブジェクトを右手系の立方体の中に，z方向を半分に圧縮して視ることになります．一般的には，\begin{center}\verb|{{0, 1}, {0, 1}, {0, 1}}|\end{center}などのように，当方的な縮尺で3Dオブジェクトを描画することが多いと思いますが，関数$z=f(x,y)$などを描画する際は，$z$方向に適当にリサイズして描画することもあると思います．そのような時，固定座標系を変更することで，3Dオブジェクトの見え方を変更することができます．

\noindent
{\bf 自由座標系による描画}

GLSC3Dでは全ての描画は自由座標系を用いて行われます\footnote{例外は \verb|g_text_standard| です．}．
...
%描画オブジェクト空間に描画オジェクトを生成する
%・自由座標系：2Dもしくは3Dオブジェクトが定義される座標系です．GLSC2Dでは仮想座標系とも呼ばれていました．


%2Dオブジェクトの場合，そのオブジェクトは何らかの平面の上で描画されますが，その平面を長方形として，\verb|{{x_0,x_1}, {y_0,y_1}}|として指定することで，2Dオブジェクトは標準座標系にマッピングされ\verb|{{x_0,x1},{y_0,y_1}}|内に描かれます．

%3Dオブジェクトの場合，そのオブジェクトは何らかの空間の上で描画されますが，その空間を直方体として，\begin{center}\verb|{{x_0,x1}, {y_0,y_1}, {z_0,z_1}}|\end{center}として指定することで，3Dオブジェクトは固定座標系へマッピングされます．

%例えば，固定座標系\begin{center}\verb|{{x_f_0,x_f_1}, {y_f_0,y_f_1}, {z_f_0,z_f_1}}|=\verb|{{0,1}, {0,1}, {0,1}}|\end{center}とします．ここで，自由座標系を\begin{center}\verb|{{x_0,x_1}, {y_0,y_1}, {z_0,z_1}}|=\verb|{{0,1}, {0,1}, {0,1}}|\end{center}とした場合，3Dオブジェクトはそのままの比率で描画されます．一方，\begin{center}\verb|{{x_0,x_1}, {y_0,y_1}, {z_0,z_1}}|=\verb|{{0,1}, {0,1}, {0,100}}|\end{center}とした場合，$z$方向へ100倍圧縮した描画結果を得ます．

%以上からわかるように，自由座標系は固定座標系とセットで定義して初めて意義が生まれます．GLSC2Dでは固定座標系は標準座標系と同一視されていましたが，GLSC3D 3.0.0以降では，この点をご注意ください．

%また，GLSC3Dでははいくつかの例外を除いて描画は自由座標系に対して行われます．（例外：\verb|g_text_standard|）．
...
視点位置: $\verb|eye| = (1.3(\verb|x_1_f|-\verb|x_0_f|), -2.4(\verb|y_1_f|-\verb|y_0_f|), 2(\verb|z_1_f|-\verb|z_0_f|))$. \\
画面上方向: $\verb|up| = (0, 0, 1)$. \\
拡大率: $\verb|zoom| = 1$. 

GLSC3Dでは，自由座標系の範囲を全て$\verb|x_0| < \verb|x_1|$のように定めると``右手系''として描画される．
ただし，奇数個の範囲を$\verb|x_0| > \verb|x_1|$のように定めると``左手系''として描画される．
偶数個であれば``右手系''として描画される．

固定座標系の範囲を全て$\verb|x_0_f| < \verb|x_1_f|$のように定めなければならない．
$\verb|x_0_f| \geq \verb|x_1_f|$のように設定するとエラーになる．
\end{itembox}

%%%%%%%%%%%%%%%%%%%ここに関数の説明に使う絵を載せる．%%%%%%%%%%%%%%%%
\begin{figure}[htb]
\centering
\includegraphics[width=50mm]{g_def_scale.png.eps}
 \hspace{0.5zw}
\includegraphics[width=50mm]{g_def_scale_left_hand.png.eps}
...
\verb|g_def_scale_3D|もしくは\verb|g_def_scale_3D_fix|で定められているデフォルトの視点位置・画面上方向・拡大率を任意に変更できる．
\end{itembox}
...
\centering
(a)
\includegraphics[width=40mm]{g_vision_eye_04.png.eps}
(b)
\includegraphics[width=40mm]{g_vision_eye_05.png.eps}
(c)
\includegraphics[width=40mm]{g_vision_eye_06.png.eps}

\vspace{0.5zw}
(d)
\includegraphics[width=40mm]{g_vision_eye_03.png.eps}
(e)
\includegraphics[width=40mm]{g_vision_eye_09.png.eps}
(f)
\includegraphics[width=40mm]{g_vision_eye_07.png.eps}

\vspace{0.5zw}
(g)
\includegraphics[width=40mm]{g_vision_eye_02.png.eps}
(h)
\includegraphics[width=40mm]{g_vision_eye_01.png.eps}
(i)
\includegraphics[width=40mm]{g_vision_eye_08.png.eps}
...
視点位置を真上や真下に設定するときには, 多少ずらさなければならない.
(f) $(1, 0, 0)$. \\
...
\centering
(a)
\includegraphics[width=40mm]{g_vision_up_01.png.eps}
(b)
\includegraphics[width=40mm]{g_vision_up_02.png.eps}
(c)
\includegraphics[width=40mm]{g_vision_up_03.png.eps}
...
\centering
(a)
\includegraphics[width=40mm]{g_vision_zoom_01.png.eps}
(b)
\includegraphics[width=40mm]{g_vision_zoom_02.png.eps}
(c)
\includegraphics[width=40mm]{g_vision_zoom_03.png.eps}
