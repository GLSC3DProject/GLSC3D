				...
				for(m = 0;m < 28;m ++)
				{
					...
					if(Edge[0].flag != 0)
					{
						...
						if(Edge[1].flag != 0)
						{
							...
							{
								r2 = Edge[2].r;
								norm = g_normalize(g_cross(r1 - r0,r2 - r0));
								g_triangle_3D_core_smooth(r0,r1,r2,norm, norm, norm, WIRE_OR_FILL);
							}
							Edge[2] = Judgeheight(r[0], r[3], v[0], v[3], height);
							if(Edge[2].flag != 0)
							{
					...
					if(SankakuShikaku != 0)
					{
						...
						if(SankakuShikaku == 4)
						{
							...
							r1 = Edge[EdgeMember[1]].r;
							r2 = Edge[EdgeMember[2]].r;
							r3 = Edge[EdgeMember[3]].r;
							norm = g_normalize(g_cross(r1 - r0,r2 - r0));
							g_triangle_3D_core_smooth(r0,r1,r2,norm, norm, norm, WIRE_OR_FILL);
							norm = g_normalize(g_cross(r2 - r0,r3 - r0));
							g_triangle_3D_core_smooth(r0,r2,r3,norm, norm, norm, WIRE_OR_FILL);
						}
					}
					 */
				}
